...
(function() {
  var commander, defaults, document, ensureDirectory, exec, ext, fs, generateDocumentation, generateHtml, getLanguage, getResource, highlight, highlightEnd, highlightStart, key, l, languages, mkdir, parse, path, resolveSource, run, showdown, spawn, template, value, version, _ref, _ref1,
  ...
  mkdir = function(fullPath, cb, made) {
    var mode;
    if (made == null) {
      made = null;
    }
    ...
    return fs.mkdir(fullPath, mode, function(er) {
      if (!er) {
        return cb(null, made || fullPath);
      }
      if (er.code === 'ENOENT') {
        return mkdir(path.dirname(fullPath), function(er, made) {
          if (er) {
            return cb(er, made);
          } else {
            return mkdir(fullPath, cb, made);
          }
        });
      }
      return fs.stat(fullPath, function(er2, stat) {
        er2 = er2 || !stat.isDirectory() ? er : null;
        return cb(er2, made);
      });
    });
  };
