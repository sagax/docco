...
(function() {
  var commander, defaults, destination, document, ensureDirectory, exec, ext, fs, generateDocumentation, generateHtml, getLanguage, getResource, hasProgram, highlightEnd, highlightJsHighlight, highlightStart, key, l, languages, parse, path, pygmentsHighlight, resolveSource, run, showdown, spawn, template, value, version, _ref, _ref1,
  ...
  generateDocumentation = function(source, config, callback) {
    return fs.readFile(source, function(error, buffer) {
      ...
      return hasProgram("pygmentize", function(pygmentsInstalled) {
        var highlight;
        highlight = pygmentsInstalled && config.highlight === "pygments" ? pygmentsHighlight : highlightJsHighlight;
  ...
  pygmentsHighlight = function(source, sections, callback) {
  ...
  highlightJsHighlight = function(source, sections, callback) {
    var hljs, i, language, output, section, text, _i, _len;
    hljs = require("highlight.js");
    hljs.LANGUAGES['coffee-script'] = hljs.LANGUAGES['coffeescript'];
    language = getLanguage(source);
    output = "";
    text = (function() {
      var _i, _len, _results;
      ...
      for (_i = 0, _len = sections.length; _i < _len; _i++) {
        section = sections[_i];
        _results.push(hljs.highlight(language.name, section.codeText).value.trim());
      }
      return _results;
    })();
    output = output.replace(highlightStart, "").replace(highlightEnd, "");
  ...
  destination = function(config) {
    ...
    return function(filepath) {
      if (config.prettyname && path.dirname(filepath) !== '.') {
        return path.join(config.output, path.dirname(filepath).replace(/\//g, '_') + '_' + path.basename(filepath, path.extname(filepath)) + '.html');
      } else {
  ...
  generateHtml = function(source, sections, config) {
    var configuredDestination, dest, html, title;
  ...
  hasProgram = function(program, callback) {
    return exec("command -v " + program + " >/dev/null 2>&1 || { echo >&2 exit 1; }", function(error, stdout, stderr) {
  ...
  run = function(args) {
    ...
    commander.version(version).usage("[options] <filePattern ...>").option("-c, --css [file]", "use a custom css file", defaults.css).option("-o, --output [path]", "use a custom output path", defaults.output).option("-t, --template [file]", "use a custom .jst template", defaults.template).option("-h, --highlight [highlighter]", "choose between \"pygments\" or \"highlightjs\"", defaults.highlight).option("-p, --prettyname", "use pretty-name").option("-m, --makeindex", "make index file").option("-i, --indexfile [index]", "index file name", defaults.index).option("-l, --indextitle [indextitle]", "title for the index", defaults.indextitle).parse(args).name = "docco";
  ...
  document = function(sources, options, callback) {
    ...
    return ensureDirectory(config.output, function() {
      var destinationFn, files, html, nextFile;
      ...
      if (config.makeindex) {
        ...
        return fs.writeFileSync(path.join(config.output, config.index), html);
      }
    });
  };
