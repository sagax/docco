(function() {
  ...
  parse = function(source, code) {
    ...
    for (_i = 0, _len = lines.length; _i < _len; _i++) {
      line = lines[_i];
      if (line.match(language.comment_enter)) incomment = true;
      if (!line.match(language.comment_filter) && (incomment || line.match(language.comment_matcher))) {
        if (has_code) {
          save(docs_text, code_text);
          has_code = docs_text = code_text = '';
        }
        if (line.match(language.comment_exit)) incomment = false;
