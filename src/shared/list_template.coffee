...
<tr class="<%= entry.type + (entry.is_markdown ? ' markdown' : '') %>">
<td class="icon"></td>
<td><a <%= entry.type == 'd' ? 'forward' : entry.is_markdown ? 'readme=\"' + entry.document + '\"' : 'href=\"' + (entry.type == 'm' ? 'https://github.com/' + entry.submodule : (entry.action === 's' ? index_path.concat(entry.document).join('/') : 'https://github.com/' + user + '/' + repo + '/blob/master/' + index_path.concat(entry.name).join('/'))) + '\"' %><%= entry.action === "g" ? "target=\'_blank\'": "" %>><%= entry.name %></a></td>
<td class="<%= size %>"><span class="hidden"><%= entry.sloc ? (entry.sloc + " " + (entry.sloc > 1 ? "lines" : "line")) : "-" %></span><span><%= entry.size %></span></td>
<td <%= (typeof entry.modified) === 'string' ? '' : 'val="' + entry.modified + '"' %>><%= (typeof entry.modified) === 'string' ? entry.modified : '' %></td>
<td class="<%= message %>"><div><span><%= entry.message %></span><span class="file_browser_author" email="<%= entry.email %>"> <%= entry.author %></span></div><div class="hidden"><%= entry.description %></div></td>
</tr>
<% }); %>
</tbody>
</table>
