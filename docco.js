...
(function() {
  ...
  write = function(source, sections, config) {
    var destination, first, hasTitle, html, qualifiedName, title;

    qualifiedName = function(file) {
      var nameParts;

      nameParts = path.dirname(file).replace(config.cwd, '').split(path.sep);
      nameParts.push(path.basename(file, path.extname(file)));
      return nameParts.join(config.seperator) + '.html';
    };
    destination = function(file) {
      return path.join(config.output, qualifiedName(file));
    };
